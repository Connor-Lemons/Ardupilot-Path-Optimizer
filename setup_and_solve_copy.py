    ...
    def setup(self):
        ...
        def objective(arg):
            ...
            for p in range(tot_phase):
                objective += arg.phase[p].final_time
            objective = W_t*objective**2
        ...
        for p in range(tot_phase):
            problem.bounds.phase[p].initial_time.lower = problem.bounds.phase[p].initial_time.upper = 0
            problem.bounds.phase[p].final_time.lower = 1
            problem.bounds.parameter.lower[3*p] = -1/ap.min_turn
            problem.bounds.parameter.upper[3*p] = 1/ap.min_turn
            problem.guess.parameter[3*p] = 1/ap.min_turn
    ...
    def extract(self):
        ...
        for p in range(tot_phase):
            ...
            if p == 0:
                ...
                t_temp = sol.phase[p].time + t_temp[-1]
            if p == 0:
